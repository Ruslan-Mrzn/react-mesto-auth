function PopupWithForm({popupName, formName, formTitle, submitButtonValue, children, isOpen, onClose, onSubmit, formIsValid}) {
  return (
    <article className={`page__popup popup popup_type_${popupName} popup_bg-opacity_medium ${isOpen ? 'popup_opened' : ''}`}>
      <div className="popup__container">
        <form className="form" name={`${formName}-form`} onSubmit={onSubmit} noValidate>
          ...
          {children}
          <button type="submit" disabled={formIsValid ? false : true}className={`form__submit-button ${formIsValid ? '' : 'form__submit-button_disabled'}`} value={submitButtonValue}>{submitButtonValue}</button>
        </form>
